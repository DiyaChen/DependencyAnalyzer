    ...
    class XML
    {
        public void XmlBuilder(List<Elem> table, string FileName)
        {
            ...
            foreach (Elem e in table)
            {
                if (e.type == "namespace")
                {
                    XElement ns = new XElement("namespace--" + e.name);            //if detected namespace, new a node to store it 
                    xml.Add(ns);
                    foreach (Elem el in table)
                    {
                        ...
                        {
                            XElement cl = new XElement(el.type + "--" + el.name);
                            ...
                            foreach (Elem ele in table)
                            {
                                ...
                                {
                                    XElement func = new XElement("function", " " + ele.name + " ");
